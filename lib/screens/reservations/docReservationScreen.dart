...
class DocReservationScreen extends StatefulWidget {
  final String clinicPass;
  final String clinicName;

  const DocReservationScreen({Key? key, required this.clinicPass, required this.clinicName}) : super(key: key);

  @override
  State<DocReservationScreen> createState() => _DocReservationScreenState();
}

class _DocReservationScreenState extends State<DocReservationScreen> {
  ...
  //   var hour = selectedTime.length <= 10? int.parse(selectedTime.split(':')[0].substring(0,1))
  //       : int.parse(selectedTime.split(':')[0].substring(0,2));
  //   var minute = selectedTime.length <= 10? int.parse(selectedTime.split(':')[1].substring(0,1))
  ...
  //   };
  //
  //   ReservationSystem().appointments(widget.clinicPass);
  //   if(datePicked != null && selectedTime.isNotEmpty){
  //     FirebaseFirestore.instance.collection('clinics').doc(widget.clinicPass).collection('reservation').add(
  //         {
  ...
  requestReservation() {
    if(selectedTime.isNotEmpty && datePicked != null && formKey.currentState!.validate()) {
      FirebaseFirestore.instance.collection('clinics').doc(widget.clinicPass).collection('requests').doc().set({
        'firstName' : firstName.text,
        'lastName' : lastName.text,
        'time' : DateTime.now(),
        'phone' : phone.text,
        ...
        'user' : user!.uid,
        'pass' : widget.clinicPass,
        'placeNme' : widget.clinicName,
      });
