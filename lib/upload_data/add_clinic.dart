...
class _AddClinicState extends State<AddClinic> {
  ...
  final storage = FirebaseStorage.instance.ref('clinic');
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: () async{
              if (formKey.currentState!.validate() && imagePicked != null) {
                var snapshot = await storage.child('${DateTime.now()}').putFile(imagePicked!);
                imageUrl = await snapshot.ref.getDownloadURL();
                await firestore.collection('clinics').doc(passwordController.text).set({
