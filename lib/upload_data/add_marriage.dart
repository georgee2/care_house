...
class AddMarriage extends StatefulWidget {
  const AddMarriage({Key? key}) : super(key: key);

  @override
  State<AddMarriage> createState() => _AddMarriageState();
}

class _AddMarriageState extends State<AddMarriage> {
  String? gender;
  String? married;
  ...
  final storage = FirebaseStorage.instance.ref('marriage');
  ...
  uploadItem() async{
    if (formKey.currentState!.validate() &&
        imagePicked != null && gender != null && married != null) {
      var snapshot = await storage.child('${DateTime.now()}').putFile(imagePicked!);
      imageUrl = await snapshot.ref.getDownloadURL();
      FirebaseFirestore.instance.collection('marriage').doc().set({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Marriage your pet"),
      ),
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    const Text('married before?  ', style: TextStyle(fontSize: 17),),
                    Container(
                      padding: const EdgeInsets.all(3),
                      decoration: BoxDecoration(
                          color: HexColor('2F409C'),
